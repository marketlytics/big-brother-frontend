...
var getFilteredQuery = function(queryObj, devices, callback) {
	var args = {};
	...
	if(typeof args.device !== 'undefined') {
		var device = devices.filter(function(device) {
			return device._id.toString() === args.device;
		});
		if(!device) {
			return callback({err: 'device not found'}, null);
		}
		query.mac = device.mac;
		callback(null, query);
	} else {
		var macs = devices.map(function(device) {
			return device.mac;
		});
		query.mac = {
			$in: macs
...
var getIncludes = function(queryObj, records, devices, callback) {
...
exports.index = function(req, res) {
	Device.find({}, function(err, devices) {
		...
		getFilteredQuery(req.query, devices, function(err, query) {
			if(err) return res.status(500).send(err);
			Record.find(query, function(err, records) {
				if(err) return res.status(500).send(err);
				//append corresponding user id and device id to each record
				getIncludes(req.query, records, devices, function(records) {
